...
const ContactList = () => {
  ...
  var API_KEY =
    'wpculy0xX5bvnzGQANma6HeSYrigRU3EOfqK1L8DB4Z7oWt2MhoSpBbd52UyurDlEVma0csHZLi837TR';
    ...
    if (filtered.length === 0) {
      ...
      return {
        ...
        data: filtered.map(i => {
          return {
            name: i.displayName,
            mobile: i.phoneNumbers[0]?.number,
  ...
  const loadsearchContacts = () => {
    Contacts.getAll().then(res => {
      setContacts(
        res.filter(
          i =>
            i.displayName?.toLowerCase().indexOf(searchText.toLowerCase()) ===
            0,
        ),
      );
      setloader(false);
